...
function! PHCursorHold()
    ...
    if (b:current_syntax == 'python') || (b:current_syntax == 'pymode')
...
" KA: added autocmd on FileType to limit it to python. Other languages might
" want to set different statuslines
autocmd FileType python set stl=%-f%r\ %2*%m%*\ \ \ \ %1*%{TagInStatusLine()}%*%=[%l:%c]\ \ \ \ [buf\ %n]
